...
**Дата:** 2026-07-21
**Статус:** реализован (Фазы 1, 2б, 3, 4, 5 + baseline-починка тестов; опц. Фаза 2 — не делали)
...
- [ ] **КП-3.** `LIST_FROM_PROJECT` пишет по-чанковый чекпоинт и продолжается с
...
- [ ] **Фаза 2 — Слой 2 (чекпоинты для `LIST_FROM_PROJECT`).**
  `task_processor.py` цикл + `RESUMABLE_TYPES`; миграция не нужна.
...
Закрыты КП-1, 2, 3б, 4, 5, 6, 7. Не сделано (опционально): **КП-3 / Фаза 2** —
по-чанковый чекпоинт для `LIST_FROM_PROJECT` (сейчас пишет только финальные
items; при паузе без чекпоинта resume начнёт заново). `ESTIMATE_OPTIMIZATION`
resume оставлен на restart (как и было решено).

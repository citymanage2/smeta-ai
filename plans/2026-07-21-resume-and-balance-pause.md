...
- [x] **Фаза 9 — видимая причина паузы (после деплоя Фазы 8, 2026-07-28).**
  Наблюдение: после деплоя `6eff993` задача обновилась (13:37 → 15:12), т.е.
  возобновление сработало, но API снова отказал и задача вернулась в `paused`.
  Диагноз был невозможен: текст паузы фиксированный («баланс API Anthropic
  исчерпан»), а на сервере запросы идут через агрегатор (`ANTHROPIC_BASE_URL`) —
  чей счёт пуст, из UI не видно; сырой ответ API оставался только в логах worker'а.
  1. `InsufficientBalanceError` несёт `status_code`/`api_message` (сырой ответ),
     `_raise_if_insufficient_balance` их заполняет.
  2. `task_processor._balance_error_detail` дописывает «Ответ API: <код> <текст>»
     в шаг прогресса при паузе (и в `process()`, и в `batch_poller`) — причина
     видна прямо в карточке задачи, в логи лезть не нужно.
  3. `claude_service.api_ping()` + `GET /admin/api-health` (админ) — пробный
     вызов `max_tokens=1` без web search: «дошло ли пополнение» сразу, вместо
     ожидания 10 минут до тика поллера. Отдаёт `verdict` (ok / no_balance / auth /
     unavailable / misconfigured), `base_url`, флаги наличия ключа и
     X-Proxy-Secret (без самих секретов) и число задач на паузе. При заданном
     `base_url` в hint прямо сказано, что баланс относится к агрегатору.
  Тесты: `tests/test_admin_api_health.py` (10/10). Гейты: полный прогон
  `350 passed` (те же 22 предсуществующих падения), ruff чист.

...
- [ ] **КП-6.** На странице задачи `paused` показывается как «⏸ На паузе» с
  пояснением и ручной кнопкой «Продолжить сейчас».
- [ ] **КП-7.** Рестарт сервера не ломает `paused`-задачи; поллер подхватывает
  их после старта.
...
Не реализован. По завершении обновить статусы фаз и записать, что сделано.

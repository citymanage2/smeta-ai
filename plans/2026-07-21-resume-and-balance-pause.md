...
- [ ] **КП-3б (приоритет).** `ESTIMATE_FROM_LIST` пишет промежуточный чекпоинт
  внутри шага 2 (после каждого чанка Claude); при обрыве/паузе в шаге 2 resume
  продолжает только по необсчитанным позициям — уже посчитанные в Claude
...
- [ ] **Фаза 2б (приоритет) — промежуточный чекпоинт в шаге 2
  ...
  Самая ценная часть: защищает ~$10 сметы от повторной оплаты токенов.

...
Реализовано целиком, все пять фаз закрыты.

Гейты:
- backend: 391 passed, 9 skipped. 18 падений — предсуществующие, проверено
  сравнением с базовой веткой в отдельном worktree (там 26 падений, из них 8 —
  артефакт worktree без собранного `frontend/dist`; остаток совпадает один в один).
  Причина — локальный Python 3.9 против проектного 3.12: `monkeypatch` модульных
  атрибутов в `test_rate_limit_retry`/`test_balance_pause` и заглушки PDF/OCR.
- `ruff check` по изменённым файлам — чисто. В репозитории есть предсуществующая
  ошибка F821 (`_date` не определён в `task_processor.py:1186`) — не трогали, это
  отдельная задача.
- frontend: `tsc --noEmit` — 0 ошибок, `vitest run` — 62 passed (8 новых).
  Скрипта `npm run lint` в проекте нет.

Добавлено сверх плана по итогам bug hunt (Stage 6):
- Дедупликация событий окном 30 минут. Сценарий: остаток «в копейки» — `api_ping`
  тратит единицы токенов и проходит, а рабочий запрос со сметой падает по балансу.
  Без дедупликации пользователь получал бы «баланс пополнен» каждые 10 минут.

Отклонение от плана (Фаза 3): курсор вынесен в ответ отдельным полем
(`{cursor, events}`), а не выводится фронтом из списка — событие может быть скрыто
по правам, и без явного курсора фронт запрашивал бы его бесконечно.

Не вошло намеренно (см. Non-Goals спеки): push-канал, e-mail/Telegram, история
уведомлений в UI, изменение интервала поллера.

...
Реализовано целиком. Слой 2 (бесконечный цикл) устранён — есть кнопка «Продолжить»,
продолжающая OCR с последней страницы. Слой 1 — пик памяти OCR срезан втрое (grayscale)
без потери точности; добавлен лог пикового RSS для диагностики. Если рестарты продолжатся
даже с grayscale — по логу `peak_rss_mb` будет видно, упирается ли процесс в лимит RAM;
дальнейшие шаги (план инстанса Render / ещё большее сжатие) решать по факту.

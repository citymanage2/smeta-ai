...
### Фаза 5 — Исправление и тестирование [x]

**Исправлено:** `TaskStatus.tsx` теперь вызывает `notify()` прямо в `fetchStatus` при обнаружении `completed`/`failed`. Ранее уведомление не приходило, потому что глобальный поллер намеренно пропускал задачи пока пользователь находился на их странице.

**Сценарии для проверки:**
- [ ] Запустить задачу → **остаться на странице статуса** → дождаться завершения → toast + звук должны прийти сразу
- [ ] Запустить задачу → **перейти на другую страницу** → дождаться завершения → toast + звук через ≤5 сек (глобальный поллер)
...
- [ ] **Звук:** перед запуском задачи кликнуть по странице — проверить что AudioContext инициализирован
- [ ] **Refresh:** создать задачу → обновить страницу → уведомление НЕ приходит (known limitation, ожидаемо)
...
- [x] Фаза 3 завершена
- [x] Фаза 4 завершена
- [x] Фаза 5 завершена (исправление бага + план тестирования)

**Что осталось:** ручная проверка сценариев из Фазы 5 пользователем.

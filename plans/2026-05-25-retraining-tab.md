...
- [x] При 200+ парах обучение запускается без ошибок
- [x] Прогресс виден на фронте (полинг каждые 3 сек — progress_updater обновляет DB каждые 5 сек)
- [x] После обучения новый запрос использует обновлённую модель (reload_model + regenerate_all_price_embeddings)
- [x] При 0 парах возвращает 400 (роутер проверяет positive_pairs < 10)

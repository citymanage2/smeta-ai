...
PUT    /tasks/{task_id}/estimate/expenses/all
       body: { overhead_pct, transport_pct, contingency_pct }
       → сохранить доп. расходы сразу во всех версиях задачи (включая те, у которых expenses_overridden=True)
...
- "Сохранить во всех версиях" → обновляет `overhead_pct/transport_pct/contingency_pct` **абсолютно во всех** версиях этой задачи, без исключений, включая те, у которых `expenses_overridden=True`
- "Сохранить только в этой версии" → обновляет только текущую версию и ставит ей флаг `expenses_overridden=True`
- Флаг `expenses_overridden=True` означает только то, что эта версия в прошлом редактировалась отдельно — но при явном нажатии "Сохранить во всех" она всё равно обновляется наравне с остальными
...
Каждая версия сметы — это отдельный список строк. После AI-оптимизации строки могут добавляться и удаляться, поэтому сравнивать по внутреннему идентификатору строки невозможно (у каждой версии свои идентификаторы).

Правило: строки выравниваются по полю `num` (номер по порядку).
- Если в обеих версиях есть строка с одинаковым `num` — они ставятся в одну строку таблицы
- Если строка есть в одной версии, но её нет в другой по `num` — в колонке другой версии показываем прочерк и помечаем строку иконкой: `[+]` появилась, `[−]` удалена
- Строки типа `section` (заголовки разделов) участвуют в выравнивании наравне с остальными

...
async def queue_health(
    ...
    if restarts is not None and restarts.starts_1h >= 2:
        # Формулировка намеренно НЕ утверждает причину: каждый деплой — это тоже
        # перезапуск, и два деплоя подряд дают ту же цифру. Первая версия текста
        # уверенно говорила «не хватает памяти» и один раз уже ввела в заблуждение
        # (30.07.2026: три старта были моими же деплоями).
        hint += (
            f" Обработчик перезапускался {restarts.starts_1h} раз(а) за час. "
            "После деплоя это норма; если деплоя не было — он умирает и встаёт "

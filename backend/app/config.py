...
class Settings(BaseSettings):
    ...
    # Смерть процесса (OOM-kill контейнера) видна по job, помеченной нашим хостом
    # и чужим pid. Столько ждём её молчания, прежде чем вернуть в очередь: больше
    # интервала heartbeat (60 с), чтобы не отобрать job у живого соседа с тем же
    # hostname. Раньше такие job ждали JOB_VISIBILITY_TIMEOUT_S = 15 минут — это и
    # видел пользователь как «задачи висят вечно».
    WORKER_CRASH_RECLAIM_AGE_S: int = 90
    # Доля лимита памяти контейнера, выше которой новую задачу не берём. Считается
    # от РЕАЛЬНОГО лимита (cgroup), а не от константы: пресет Timeweb может быть
    # любым, а прежний порог в мегабайтах угадывал его вслепую.
    WORKER_MEM_HIGH_RATIO: float = 0.8
    # Сколько памяти закладываем на одну задачу при расчёте числа слотов из лимита
    # контейнера. Замер 30.07.2026: только парсинг гранд-сметы на 20 000 строк даёт
    # +47 МБ при пике процесса 207 МБ; дальше добавляются ответы ИИ и накопленные
    # позиции. 400 МБ — консервативная оценка «тяжёлой» задачи.
    WORKER_TASK_MEM_MB: int = 400
    # Разнос захватов: пока самая свежая задача в работе младше этого, новую не
    # берём. Тормоз по памяти смотрит на ТЕКУЩУЮ занятость, а три задачи
    # захватывались за 2–4 с — когда память ещё не выросла, и тормоз не срабатывал
    # ни разу. Разнос даёт памяти первой задачи проявиться до захвата второй.
    WORKER_CLAIM_STAGGER_S: int = 60

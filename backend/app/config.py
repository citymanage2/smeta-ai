...
class Settings(BaseSettings):
    ...
    # Суммарный потолок = (POOL_SIZE + MAX_OVERFLOW) × число процессов, и он ОБЯЗАН
    # влезать в max_connections managed-БД: в логе 29.07.2026 база отвечала
    # `Connection reset by peer` на новое соединение. Overflow снижен 10 → 5:
    # web ≤ 10, worker ≤ 11 (см. docker-compose.yml), то есть ≈21 против прежних 31.
    # Фактический лимит и запас видно кнопкой «Проверить сейчас» в админке.
    DB_POOL_SIZE: int = 5

...
class Settings(BaseSettings):
    ...
    # Отдельный админский ключ (sk-ant-admin01-...) из Console — только для
    # официального отчёта о тратах (cost_report). Рабочий ключ сервиса на
    # админские эндпоинты не пускают. Пусто → остаток считается по своему
    # журналу вызовов, без сверки с Anthropic.
    ...
    # Сколько job worker гонит одновременно. ОДНА — сознательное решение
    # 30.07.2026: параллельная обработка на этой машине не выживала. Несколько
    # задач насыщали процессор (web на той же машине перестал отвечать), heartbeat
    # живого прогона опаздывал, реклейм выдавал ВТОРОЙ прогон той же задачи — и
    # петля раскручивалась до полной непроходимости, попутно оплачивая ИИ дважды.
    # Последовательная очередь считает столько же работы, но предсказуемо, и место
    # в очереди можно честно показать пользователю.
    # Поднимать это число имеет смысл только вместе с ядрами (см. _cpu_slots_cap).
    ...
    # Сколько ядер отдаём ONNX под эмбеддинги. 0 — как решит onnxruntime (он берёт
    # все). Ограничение нужно, потому что web живёт на той же машине: 30.07.2026
    # несколько задач разом заняли процессор целиком, и интерфейс перестал
    # отвечать — страница задачи не загружалась вообще.
    EMBEDDING_THREADS: int = 0
    # Дообучение модели эмбеддингов. Выключено, и это предохранитель, а не
    # заглушка: обученная модель пишется во временную папку контейнера
    # (`/tmp/smeta-finetuned`), а на Timeweb постоянных томов нет — при
    # перезапуске она исчезает. Но эмбеддинги всего прайса к тому моменту уже
    # пересчитаны ею и остаются в базе. Дальше поиск цен сравнивает векторы,
    # посчитанные разными моделями, — величины несопоставимые, подбор цены
    # деградирует и никак этого не показывает.
    # Включать только вместе с постоянным хранением модели (S3) и отметкой,
    # какой моделью посчитаны векторы прайса —
    # plans/2026-08-07-obuchenie-na-korrektirovkah.md, Фаза 6.
    RETRAINING_ENABLED: bool = False
    # Как часто обработчик вправе писать один и тот же диагностический сигнал в
    # system_events (память, сбой heartbeat). Раньше жалоба на память писалась не
    # чаще раза в 30 минут — за это время задача успевала умереть без единого
    # замера. 5 минут: следов достаточно, нагрузки нет.
    WORKER_EVENT_THROTTLE_S: int = 300
    ...
    # Неприкосновенный запас свободной памяти. Пока свободного меньше — новую
    # задачу не берём. Главный предохранитель там, где личного лимита у контейнера
    # НЕТ: 30.07.2026 на проде оказалось именно так (3,9 ГБ делят web и обработчик),
    # и «доля от лимита» без этой проверки ничего не защищала.
    WORKER_MEM_MIN_FREE_MB: int = 512

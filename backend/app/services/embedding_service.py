...
def normalize_name(text: str) -> str:
    ...
    - Марки материалов: М-100, М 100, M100 → м100
    - Двойные пробелы убираются, текст trim-ится
    - Результат — нижний регистр для сравнения

    **Склеивать разрешено только отдельно стоящую букву с числом.** До
    02.09.2026 правило марок съедало и слова между ними
    (`([буква])...(?:[слово]\s+)*(\d+)`), и от названия оставался огрызок:
    «Пленка гидроизоляционная Изоспан A 1,6х50 м» и «Пленка пароизоляционная
    Изоспан B 1,6х50 м» превращались в одно и то же «п1,6х50 м». А это текст,
    из которого считается вектор позиции, и ключ «такая позиция уже есть» при
    загрузке прайса файлом: пароизоляция получала цену гидроизоляции, ГКЛО —
    цену ГКЛВ, ЦЕМ I — цену ЦЕМ II, и пять планок кровли из шести пропадали
    при записи. Каждая такая пара закреплена тестом
    (`tests/test_normalize_name_collisions.py`).
    """
    if not text:
        return ""
    ...
    # Буква должна стоять отдельно: «М 100» — марка, а «до 32мм» и «ГКЛО 2500»
    # — нет, там буква внутри слова и склеивать её с числом нельзя.
    result = re.sub(

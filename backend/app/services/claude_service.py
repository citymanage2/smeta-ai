...
_COST_PER_TOKEN: dict[str, dict[str, float]] = {
    # Действующая цена Sonnet 5. До 01.09.2026 здесь намеренно стояла завышенная
    # $3/$15: вводная цена $2/$10 была объявлена «до 31.08.2026», и завышение
    # страховало от объявленного повышения с 1 сентября. Повышения не будет —
    # Anthropic объявил вводную цену стандартной, — и страховка превратилась бы в
    # завышение расхода ровно в полтора раза. Для дашборда это была безопасная
    # сторона ошибки, для остатка денег — опасная: сервис считал бы, что денег на
    # треть меньше, чем есть (plans/2026-09-01-ostatok-deneg-na-api.md, Фаза 1).
    "claude-sonnet-5": {
        "input": 2.0 / 1_000_000,
    ...
    # Неизвестная модель — считаем по цене Sonnet 4.6, а не рабочей Sonnet 5.
    # Занижать нельзя: недосчитанный расход молча съедает остаток на счёте, и
    # исчерпанный баланс приходит без предупреждения. Переплатить в оценке
    # безопаснее, чем недооценить.
    "default": {
        "input": 3.0 / 1_000_000,
        "output": 15.0 / 1_000_000,
        "cache_read": 0.30 / 1_000_000,
